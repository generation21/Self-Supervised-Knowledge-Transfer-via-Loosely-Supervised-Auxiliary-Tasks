...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, sourceKind = 'places365',isSource = True, multi_source = True, transfer_module=True):
        ...
        if self.isSource: 
            if self.transfer_module:                
                if self.multi_source: 
                    ...
                    self.auxiliary = aggregation_feature(512 * block.expansion, auxi_class)
                ...
                if multi_source:
                    ...
                    self.auxiliary = nn.Linear(512 * block.expansion, auxi_class)
    ...
    def _forward_impl(self, x):
        ...
        if self.transfer_module:
            if self.multi_source:   
                auxiliary_output1 = self.auxiliary_imagenet(b1, b2, b3, b4)
                auxiliary_output2 = self.auxiliary_places(b1, b2, b3, b4)
            else: 
                if self.sourceKind == 'imagenet':
                    auxiliary_output1 = self.auxiliary(b1, b2, b3, b4)
                elif self.sourceKind == 'places365':
                    auxiliary_output2 = self.auxiliary(b1, b2, b3, b4)
        else:
            if self.multi_source:
        ...
        return x, auxiliary_output1, auxiliary_output2
